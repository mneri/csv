...
public class CityPopDeserializer implements CsvDeserializer<CityPop> {
    ...
    public CityPop deserialize(RecyclableCsvLine line) {
        ...
        try {
            city.setPopulation(Integer.parseInt(line.getString(4)));
        } catch (Exception ignored) {
        }
        try {
            city.setLatitude(Double.parseDouble(line.getString(5)));
        } catch (Exception ignored) {
        }
        try {
            city.setLongitude(Double.parseDouble(line.getString(6)));
        } catch (Exception ignored) {
        }
        //@formatter:on
