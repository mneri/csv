...
class Rfc4180FullyRelaxedFormatTest extends Specification {
    def driver = new FormatDriver(Rfc4180FullyRelaxedFormat.provider())
    ...
    def "never throws, on every input of up to 6 characters made of a, comma, quote, CR and LF"() {
        given:
        def alphabet = ["a", ",", '"', "\r", "\n"]
        def inputs = [""]
        def longest = [""]
    ...
    def "every field survives quoting, with #name line breaks"() {
        given:
        def values = ["", "a", " ", ",", '"', '""', "\n", "\r", "\r\n", 'a"b', '"a', 'a"', "a,b", '","', "a\nb"]
        def rows = []
        ...
        when:
        def input = rows.collect { row -> row.collect { '"' + it.replace('"', '""') + '"' }.join(",") }.join(lineBreak)
